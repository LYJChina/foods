...
class AsyncParseTask:
    ...
    def update_progress(self, update: dict[str, Any]) -> None:
        """Update progress reported by the backend without failing the parse job."""
        ...
        if isinstance(total_pages, int) and total_pages > 0 and isinstance(completed_pages, int):
            file_percent = max(0.0, min(100.0, completed_pages * 100.0 / total_pages))
            progress["file_percent"] = round(file_percent, 1)
            if isinstance(file_index, int) and isinstance(file_count, int) and file_count > 0:
                overall_percent = ((max(0, file_index - 1) + file_percent / 100.0) / file_count) * 100.0
                progress["percent"] = round(max(0.0, min(100.0, overall_percent)), 1)
...
class AsyncTaskManager:
    ...
    async def _run_task(self, task: AsyncParseTask) -> None:
        ...
        task.update_progress(
            {
                "phase": "initializing",
                "message": "Task is preparing the document",
            }
        )
