...
def _add_ocr_metadata(para_content, para_block):
    """Expose external VLM OCR metadata in content-list output when available."""
    spans = []

    def visit(value):
        if isinstance(value, dict):
            if value.get('type') == ContentType.TEXT:
                spans.append(value)
    ...
    visit(para_block)
    scores = [span['score'] for span in spans if isinstance(span.get('score'), (int, float))]
    char_boxes = [span['char_boxes'] for span in spans if isinstance(span.get('char_boxes'), list)]
    if scores:
        para_content['confidence'] = float(sum(scores) / len(scores))
    output_boxes = []
    for span_boxes in char_boxes:
        for item in span_boxes:
            if not isinstance(item, dict) or not isinstance(item.get('bbox'), list):
                continue
            output_item = dict(item)
            output_item['bbox'] = [
                int(round(coord * 1000)) if 0 <= coord <= 1 else coord
                for coord in item['bbox']
            ]
            output_boxes.append(output_item)
    if output_boxes:
        para_content['char_boxes'] = output_boxes
    ...
    if para_type in [
        ...
        for block in para_block['blocks']:
            if block['type'] == BlockType.CODE_BODY:
                para_content[BlockType.CODE_BODY] = _render_code_block_markdown(block, para_block)
            if block['type'] == BlockType.CODE_CAPTION:
                para_content[BlockType.CODE_CAPTION].append(merge_para_with_text(block))

    _add_ocr_metadata(para_content, para_block)
