    ...
    def __init__(self, page_blocks: list, width, height):
        ...
        for index, block_info in enumerate(page_blocks):
            ...
            if raw_block_type == "text":
                ocr_score = block_info.get("ocr_score")
                char_boxes = block_info.get("char_boxes")
                spans_to_annotate = span if isinstance(span, list) else [span]
                for text_span in spans_to_annotate:
                    if not isinstance(text_span, dict) or text_span.get("type") != ContentType.TEXT:
                        continue
                    if isinstance(ocr_score, (int, float)):
                        text_span["score"] = float(ocr_score)
                    if isinstance(char_boxes, list):
                        text_span["char_boxes"] = char_boxes
