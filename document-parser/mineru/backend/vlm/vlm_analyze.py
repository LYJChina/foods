...
from .external_layout_adapter import (
    aio_batch_extract_with_bbox_json_layout,
    aio_batch_extract_with_native_text_fallback,
    batch_extract_with_bbox_json_layout,
    batch_extract_with_native_text_fallback,
)
...
from mineru.utils.multimodal_config import (
    PDF_NATIVE_TEXT_FAST_PATH,
    PDF_NATIVE_TEXT_MIN_CHARS,
    get_fixed_http_client_settings,
)
    ...
    def get_model(
        ...
        **kwargs,
        ...
        if backend == "http-client":
            fixed_settings = get_fixed_http_client_settings()
            server_url = fixed_settings.server_url
            kwargs["model_name"] = fixed_settings.model_name
            kwargs["max_concurrency"] = fixed_settings.max_concurrency
            kwargs["http_timeout"] = fixed_settings.http_timeout
            kwargs["server_headers"] = fixed_settings.server_headers
            kwargs["skip_model_name_checking"] = fixed_settings.skip_model_name_checking

        model_name = kwargs.get("model_name")
        skip_model_name_checking = kwargs.get("skip_model_name_checking", False)
        key = (backend, model_path, server_url, model_name)
        with self._lock:
            ...
            for window_index, window_start in enumerate(range(0, page_count, effective_window_size or 1)):
                ...
                try:
                    ...
                    with predictor_execution_guard(predictor):
                        if backend == "http-client":
                            if PDF_NATIVE_TEXT_FAST_PATH:
                                window_results = batch_extract_with_native_text_fallback(
                                    predictor,
                                    images_pil_list,
                                    pdf_doc,
                                    window_start,
                                ...
                                window_results = batch_extract_with_bbox_json_layout(
                                    predictor,
                                    images_pil_list,
                                    image_analysis=image_analysis,
                                )
            ...
            for window_index, window_start in enumerate(range(0, page_count, effective_window_size or 1)):
                ...
                try:
                    ...
                    async with aio_predictor_execution_guard(predictor):
                        if backend == "http-client":
                            if PDF_NATIVE_TEXT_FAST_PATH:
                                window_results = await aio_batch_extract_with_native_text_fallback(
                                    predictor,
                                    images_pil_list,
                                    pdf_bytes,
                                    window_start,
                                ...
                                window_results = await aio_batch_extract_with_bbox_json_layout(
                                    predictor,
                                    images_pil_list,
                                    image_analysis=image_analysis,
                                )
